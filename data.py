...
data_headers = {
    ...
    "int_rate":                1, # Interest Rate on the loan.
    "installment":             2, # The monthly payment owed by the borrower if the loan originates.
    "emp_length":              3, # Employment length in years. Possible values are between 0 and 10 where 0 means less than one year and 10 means ten or more years. 
    "home_ownership":          4, # The home ownership status provided by the borrower during registration or obtained from the credit report. Values are: RENT, OWN, MORTGAGE, OTHER.
    "annual_inc":              5, # The self-reported annual income provided by the borrower during registration.
    "verification_status":     6, # Indicates if income was verified by LC, not verified, or if the income source was verified.
    "purpose":                 7, # A category provided by the borrower for the loan request. 
    "dti":                     8, # Ratio calculated using the borrower’s total monthly debt payments on the total debt obligations, excluding mortgage and the requested LC loan, divided by the borrower’s self-reported monthly income.
    "delinq_2yrs":             9, # The number of 30+ days past-due incidences of delinquency in the borrower's credit file for the past 2 years.
    "inq_last_6mths":         10, # The number of inquiries in past 6 months (excluding auto and mortgage inquiries).
    "mths_since_last_delinq": 11, # The number of months since the borrower's last delinquency.
    "mths_since_last_record": 12, # The number of months since the last public record.
    "open_acc":               13, # The number of months since the last public record.
    "pub_rec":                14, # The number of derogatory public records.
    "total_acc":              15, # The total number of credit lines currently in the borrower's credit file.
    "pub_rec_bankruptcies":   16, # Number of public record bankruptcies.
...
def get_train_test_split(data, random_state=0):
  X = data[:,:-2]
